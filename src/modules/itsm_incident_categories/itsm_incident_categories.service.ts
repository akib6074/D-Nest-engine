...
@Injectable()
      export class ItsmIncidentCategoriesService {
        constructor(
          @InjectModel(ItsmIncidentCategories)
          private itsm_incident_categories: typeof ItsmIncidentCategories,
       ...
       async create(createItsmIncidentCategoriesDto: CreateItsmIncidentCategoriesDto, payload: any,assets: any) {
        
          // const thisTableInfo = await this.adeTables.findOne({where: { table_name: 'itsm_incident_categories',is_active:true, }});
          ...
          if (assets.length > 0) {
            ...
            await this.itsm_incident_categories.create({
              ...createItsmIncidentCategoriesDto,
          ...
          return ['one itsm_incident_category added!'];          
  }
    ...
    const attributesInclude = iattributes ? JSON.parse(iattributes) : null;    
      ...
      //   where: { table_name: 'itsm_incident_categories',is_active:true, },
      // });
      // if (!thisTableInfo) throw new ForbiddenException();
      ...
      const data = await this.itsm_incident_categories.findAndCountAll({        
        order: [['id', 'DESC']],
      ...
      this.helpers.flattenObject(m.get({ plain: true }),'itsm_incident_categories'),
    );
      const response = this.helpers.getPagingData(isDropDown
        ...
        : { count: count, rows: plain }, page, limit,'itsm_incident_categories');
      return response || {};
    ...
    async findOne(id: number, payload: any) {       
      ...
      // });
      // if (!canRead) throw new UnauthorizedException();
      const response = await this.itsm_incident_categories.findOne({
            where: {
              id,              
            },
            attributes: {
  ...
  async update(id: number, updateItsmIncidentCategoriesDto: UpdateItsmIncidentCategoriesDto,payload: any,assets: any) {   
      // const thisTableInfo = await this.adeTables.findOne({
      //   where: { table_name: 'itsm_incident_categories',is_active:true, },
      // });
      // if (!thisTableInfo) throw new ForbiddenException();
      ...
      if (assets.length > 0) {
        const file_name = assets[0].filename
        const photo = process.env.BASE_URL + 'images/users/' + file_name;            
        await this.itsm_incident_categories.update({
          ...updateItsmIncidentCategoriesDto,
    ...
    return ["itsm_incident_categories updated!"];
  ...
  async remove(id: number, payload: any) {     
      ...
      const response = await this.itsm_incident_categories.update(
            {
                is_active: 0,
            ...
            return ["one record deleted from itsm_incident_categories!"];
    }
  }

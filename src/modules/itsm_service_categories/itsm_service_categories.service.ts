...
@Injectable()
      export class ItsmServiceCategoriesService {
        constructor(
          @InjectModel(ItsmServiceCategories)
          private itsm_service_categories: typeof ItsmServiceCategories,
       ...
       async create(createItsmServiceCategoriesDto: CreateItsmServiceCategoriesDto, payload: any,assets: any) {
        
          // const thisTableInfo = await this.adeTables.findOne({where: { table_name: 'itsm_service_categories',is_active:true, }});
          ...
          if (assets.length > 0) {
            ...
            await this.itsm_service_categories.create({
              ...createItsmServiceCategoriesDto,
          ...
          return ['one itsm_service_category added!'];          
  }
    ...
    const attributesInclude = iattributes ? JSON.parse(iattributes) : null;    
      ...
      //   where: { table_name: 'itsm_service_categories',is_active:true, },
      // });
      // if (!thisTableInfo) throw new ForbiddenException();
      ...
      const data = await this.itsm_service_categories.findAndCountAll({        
        order: [['id', 'DESC']],
      ...
      this.helpers.flattenObject(m.get({ plain: true }),'itsm_service_categories'),
    );
      const response = this.helpers.getPagingData(isDropDown
        ...
        : { count: count, rows: plain }, page, limit,'itsm_service_categories');
      return response || {};
    ...
    async findOne(id: number, payload: any) {       
      ...
      // });
      // if (!canRead) throw new UnauthorizedException();
      const response = await this.itsm_service_categories.findOne({
            where: {
              id,              
            },
            attributes: {
  ...
  async update(id: number, updateItsmServiceCategoriesDto: UpdateItsmServiceCategoriesDto,payload: any,assets: any) {   
      // const thisTableInfo = await this.adeTables.findOne({
      //   where: { table_name: 'itsm_service_categories',is_active:true, },
      // });
      // if (!thisTableInfo) throw new ForbiddenException();
      ...
      if (assets.length > 0) {
        const file_name = assets[0].filename
        const photo = process.env.BASE_URL + 'images/users/' + file_name;            
        await this.itsm_service_categories.update({
          ...updateItsmServiceCategoriesDto,
    ...
    return ["itsm_service_categories updated!"];
  ...
  async remove(id: number, payload: any) {     
      ...
      const response = await this.itsm_service_categories.update(
            {
                is_active: 0,
            ...
            return ["one record deleted from itsm_service_categories!"];
    }
  }
